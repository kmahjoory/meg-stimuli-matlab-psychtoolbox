...
function [T_shuffled] = mk_one_block_training_sigmoid()

%{
dfm = 0.1
[sigs, triggers] = mk_one_block(dfm);
fs = 44100;
...
%write_path = '';
fs = 44100;
mod_freq_all = 2.5;%[1, 1.5,  2,  2.5, 3, 3.5, 4];
%dfm_all = [-10.5:3:10.5]/100;%[-abs(dfm), abs(dfm)];
dfm_all = [-10.5:3:-4.5 4.5:3:10.5]/100;

phase_ref_all = [0];
phase_targ_all = [0];
%freq_match_all = ['increase', 'decrease'];
nreps = 7;
ntrials_in_block = length(mod_freq_all)* length(dfm_all)* length(phase_ref_all)* length(phase_targ_all) * nreps;
